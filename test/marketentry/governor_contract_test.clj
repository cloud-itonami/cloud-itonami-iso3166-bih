  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Fabrikacija d.o.o."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Fabrikacija d.o.o." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest tax-social-security-arrears-is-held-and-unoverridable
  (testing "unpaid tax/social-security arrears with NO confirmed reprogram agreement -> HARD hold (flagship check, no de-minimis carve-out for this jurisdiction)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:tax-social-security-arrears} (-> (store/ledger db) last :basis)))
...
(deftest jib-unverified-is-held-and-unoverridable
  (testing "unverified JIB (jedinstveni identifikacioni broj) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:jib-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest reprogrammed-confirmed-arrears-does-not-block-submit
  (testing "a CONFIRMED reprogrammed/deferred-payment arrangement (ZJN Čl. 45(3)) is accepted -- eng-6 has arrears but a confirmed reprogram agreement"
    (let [[db actor] (fresh)
          _ (assess! actor "t7bpre" "eng-6")
          _ (draft! actor "t7bpre" "eng-6")
          r1 (exec-op actor "t7b" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :interrupted (:status r1)) "governor-clean -> still escalates for human approval (actuation gate)")
