...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (BIH, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up tax-social-security-arrears) ==")
    ...
    (println "== filing/submit eng-4 (tax-social-security-arrears, no reprogram confirmed -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up jib-unverified) ==")
    ...
    (println "== filing/submit eng-5 (jib-unverified -> HARD hold) ==")
