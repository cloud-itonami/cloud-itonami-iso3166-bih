(ns marketentry.marketentryllm
  ...
  the Bosnia and Herzegovina (BIH) public-sector market-entry
  compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "jib-verified?=" (:jib-verified? e)
                        " tax-arrears-amount=" (:tax-arrears-amount e)
                        " tax-arrears-reprogrammed-confirmed?=" (:tax-arrears-reprogrammed-confirmed? e)
